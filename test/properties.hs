...
import qualified Data.BitMap     as R
...
-- | 'toAscList' produces list of correct length.
prop_toAscList_length :: [Word32] -> Bool
prop_toAscList_length l =
    let l' = nub l
    in length l' == length (R.toAscList (R.fromList l'))
...
-- | 'toDescList' produces list of correct length.
prop_toDescList_length :: [Word32] -> Bool
prop_toDescList_length l =
    let l' = nub l
    in length l' == length (R.toDescList (R.fromList l'))
...
-- | 'toAscList' and 'toDescList' produce the same list, but for order.
prop_toAscList_toDescList_agree :: NonEmptyList Word32 -> Bool
prop_toAscList_toDescList_agree (NonEmpty l) =
    let s = R.fromList l
    in (R.toAscList s) == (reverse $ R.toDescList s)
